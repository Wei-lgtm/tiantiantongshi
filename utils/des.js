...
class des {
  constructor() {
    this.keyValue = '2d5422cb1894463fb645a523d9ec7f5f';
  }
   ...
   */
  doDes(beinetkey, message, encrypt, mode, iv) {
  ...
  /// k     密钥
  encMe(s, k) {
    return this.stringToHex(this.doDes(k, s, 1, 0));
  }
  ...
  /// k     密钥
  uncMe(s, k) {
    return this.doDes(k, HexTostring(s), 0, 0);
  }
  jsencrypt(message, key) {
    var ciphertext = this.stringToHex(this.doDes(key, message, 1, 0, ""));
    return ciphertext;
  }
  //JS DES解密函数
  jsdecrypt(message, key) {
    var plaintext = this.doDes(key, this.HexTostring(message), 0, 0, "");
    return plaintext;
  }

  //设置cookie方法
  setCookies(key, val, time) {
  ...
  jsencryptToken(message, token, key) {
    var ciphertext = this.stringToHex(this.doDes(token || key, message, 1, 0, ""));
    return ciphertext;
  }

  //JS DES解密函数
  jsdeLocalcrypt(message, key) {
    var plaintext = this.doDes(key, this.HexTostring(message), 0, 0, "");
    return plaintext;
  }
}
export default new des();
